...
export const AllMembers = ({ membersOnLoad }) => {
  ...
  const cropRecords = (_members, _page) => {
    setTotalPages(Math.ceil(_members.length / RECORDS_PER_PAGE));
    paginate(_members, _page);
  };

  useEffect(() => {
    cropRecords(membersOnLoad);
  }, [currentPage]);

  useEffect(() => {
    cropRecords(membersOnLoad, 1);
  }, [membersOnLoad]);
